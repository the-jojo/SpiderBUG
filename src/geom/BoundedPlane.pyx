...
from src.geom.Node cimport Node, _is_point_btw, _dist_3d
...
cdef cnp.ndarray _get_plane_normal(cnp.ndarray[double, ndim=1] plane_lower_a,
    ...
    """calculate the plane normal vector from lower points and plane direction"""
...
cdef double _get_plane_constant(cnp.ndarray[double, ndim=1] plane_lower_a,
    ...
    """calculate the plane constant from lower points and plane normal vector"""
...
cdef cnp.ndarray _get_intersect_btw_plane_and_ray(cnp.ndarray[double, ndim=1] plane_lower_a,
    ...
    The return value is not None if the intersection is found and
    this value gives the distance along the ray. None
    implies that the intersection was either not successful or the
    intersection point was before the origin.
    :return: intersect 1-dimensional ndarray or None
    """
...
cdef cnp.ndarray _get_intersect_btw_plane_and_lseg(cnp.ndarray[double, ndim=1] plane_lower_a,
    ...
    """
    Finds the intersection point between a plane and a line segment.
    Turns the line segment into a ray to call the '_get_intersect_btw_plane_and_ray()' function
    to reuse code and then checks if the point is between the start and end points of the line segment.
    :return: intersect 1-dimensional ndarray or None
    """
    # turn to ray
...
cdef class BoundedPlane:
    """
    Represents a plane bounded by left and right lines as well as lower 2 points but is open at the top.
    The plane is defined by the lower two points a and b, the direction vector of the plane (pointing upwards),
    the plane normal (perpendicular to the plane) and the plane constant (slope)
    """
