...
class ObstacleSegment:
    ...
    # should be ok
    def __find_corresponding__(self, new_sensed_points, tolerance): # -> np.ndarray or None [[Node]]
    ...
    def update(self, new_sensed_points, was_pasued): #  -> [Node] or None [[Node]]
        ...
        if new_boundary_points is None:
            ...
            return None
        # update this obstacle's velocity

        def to_vel(a, b):
            x = (b - a).as_ndarray()
            x[2] = new_time - self.last_update_time
            return x

        if not was_pasued:
            new_vs = [to_vel(a, b) for a, b in zip(self.boundary_points, new_boundary_points)]
            #self.velocity.extend(new_vs)
    ...
    def get_avg_v_2d(self): #  -> Node
        x = np.mean(self.velocity, axis=0)
        x[2] = 0
        #return Node.from_list([-0.0, 0, 0.0]) # Node.from_list([0, 0.1, 0.0]).as_unit_vector()
        return Node.from_array(x).as_unit_vector()

    def get_avg_v_3d(self): #  -> Node
        ...
        if n.y() < -0.1:
            ...
            n.y(0.1)
        return n
        #return Node.from_list([0, 0.0, 1.0]) # Node.from_list([0, 0.1, 1.0])#.as_unit_vector()
    ...
    # maybe ok
    def get_tangent_points_3d(self, rob_cone, tangent_dist_out, tangent_dist_in, logger): #  -> [Node]
        res = [] # : [Node]
        ...
        for p_node in discontinuity_points:
            ...
            for p_int_3d in p_ints:
                ...
                if tangent_dist_out >= rob_dist > tangent_dist_in:
                    ...
                    # raise RuntimeError("Robot inside inner tangent distance")
    ...
    def get_intersect_with_path_3d(self, rob_pos_3d, path_2d, rob_speed, tangent_dist_in, only_hard_int=False): #  -> Node or None
    ...
    # maybe ok
    def get_tangent_points_2d(self, cur_pos, tangent_dist_out, tangent_dist_in, logger): #  -> [Node]
        """draw a circle with radius TANGENT_DISTANCE around the points of discontinuity,
           eliminate tangent point closer to center"""
        res = [] # : [Node]

        c_p = self.get_center()
        # loop points of discontinuity
        for d_p in [self.boundary_points[0], self.boundary_points[-1]]: # changed from x to [0
    ...
    # should be ok
    def get_intersect_with_lseg(self, l_seg, tangent_dist_out, tangent_dist_in): # -> Node or None
